...
#    gestures_and_speech_info = gestures_and_speech_info[gestures_and_speech_info['type'] == 'iconic'].reset_index(drop=True)
   ...
   for row in tqdm(gestures_and_speech_info.itertuples(), total=num_gestures):
      ...
      from_ts = row.from_ts
      to_ts = row.to_ts
      start_frame = int(from_ts*24)
      end_frame = int(to_ts*24)
      ch = row.pair_speaker
      # file = f"poses-{ch}"
    #   
    ...
    #      end_frame += 2
      ...
      if end_frame - start_frame > 72:
         ...
         start_index = start_frame + shift
         end_index = start_index + 72
      else:
         start_index = start_frame
         end_index = end_frame
      # try:
      skel = processed_keypoints_dict[ch][:, start_index:end_index, :, :][:, :, SELECTED_KEYPOINTS, :]
      mirrored_skel = mirrored_keypoints_dict[ch][:, start_index:end_index, :, :][:, :, SELECTED_KEYPOINTS, :]
    ...
    #   if speaker == 'B':
      ...
      with torch.no_grad():
         mm_outputs = jointsformer(
               skel_tensor, eval=True, utterances=[row.words],
...
def load_keypoints_dict():
   ...
   for file in glob.glob('data/selected_poses/*.npy'):
    ...
    #   pair_speaker = f"{pair}_{speaker}"
      ch = file.split("-")[1].replace(".npy", "")  # get filename without path and extension
      # name = name.split('-')
      # chapter = name[1]
      # start_frame = name[3]

      # try:
      processed_keypoints_dict[ch], mirrored_keypoints_dict[ch] = process_poses(np.load(file)) #np.load(keypoints_path)
      # except Exception as e:
      #     error_ticker += 1
         # print(e)
...
def load_models():
    ...
    bert_model = AutoModel.from_pretrained("google-bert/bert-base-cased", output_hidden_states=True).to(DEVICE)
    bert_model.eval()
...
def main():
    ...
    gestures_info_exploded = pd.read_csv('./speaker_segments.csv')
   ...
   #  gestures_info = gestures_info[gestures_info['type'] == 'iconic'].reset_index(drop=True)
    ...
    gestures_info_exploded['semantic_embeddings'] = gestures_info_exploded['words'].apply(
   ...
   #  explode the referent_clean column with '+' as separator
    ...
    model_types = {
        'multimodal-x-skeleton-semantic': 
            {'modalities': ['skeleton', 'semantic'], 'hidden_dim': 256, 'cross_modal':True,'attentive_pooling':False, 'model_weights':  'workdir/multimodal-x_skeleton_semantic_lr_0.001_batch_10_temperature_0.1_l_semantic/2025-11-14_14_35_02_393732/checkpoints/epoch=36-val/combined_loss=1.17.ckpt'},
