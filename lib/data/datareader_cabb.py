...
class CABBFeeder(Dataset):
    def __init__(
            ...
            poses_path='./data/',
            modalities=["skeleton", "semantic", "speech"],
    ...
    def load_skeletal_data(self):
        ...
        if self.filter_text:
            ...
            self.data = self.data[self.data['segmentation method'] != 'skeleton model']  # exclude the rows with segmentation method == 'skeleton model'
            if self.task == 'segmentation' or self.use_only_small_dataset:
                self.data = self.data[self.data['segmentation method'] == 'manual']
        # convert from_ts and to_ts to frames
        # self.data['start_frames'] = (self.data['from_ts'] * self.fps).astype(int)
        # self.data['end_frames'] = (self.data['to_ts'] * self.fps).astype(int)
    ...
    def __getitem__(self, index):
        ...
        # except Exception as e:
        #     print("Error in getting the utterance, setting it to 'geen speech' and error is:", e)
        #     item['utterance'] = 'geen speech'
