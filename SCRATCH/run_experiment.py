...
def main():
    ...
    pose_data, audio_data = load_raw_files(chapters)
    print(f"Loaded data for {len(chapters)} chapters.")
    ...
    all_categories = sorted(segments_df['category'].unique())
    num_folds = len(all_categories)
    print(f"\nFound {num_folds} unique categories, preparing for {num_folds}-fold cross-validation.")
    ...
    for i, val_category in enumerate(all_categories):
    for i, (train_chapter_indices, val_chapter_indices) in enumerate(kf.split(all_chapters)):
        ...
        print("\n" + "="*50)
        print(f"FOLD {i+1}/{num_folds}: Validating on category '{val_category}'")
        print(f"FOLD {i+1}/{num_folds}: Validating on chapters {val_chapters}")
        print("="*50)

        # 3a. Split categories for this fold
        train_categories = [cat for cat in all_categories if cat != val_category]
        ...
        train_indices, train_labels = parse_csv_windows(segments_df, chapters, train_categories, FPS)
        val_indices, val_labels = parse_csv_windows(segments_df, chapters, [val_category], FPS)
        ...
        if not val_indices:
            print(f"Skipping fold {i+1} as there is no data for validation category '{val_category}'.")
        ...
        train_ds_raw = RawExtractionDataset(pose_data, audio_data, train_indices, train_labels, WINDOW_SIZE)
        val_ds_raw = RawExtractionDataset(pose_data, audio_data, val_indices, val_labels, WINDOW_SIZE)
