...
NUM_CLASSES = 2
REDUCED_DIMENSION = 64        # Reduce 256 -> 64 via PCA
...
def parse_csv_windows(categories_randomized, segments_df, chapters, fps):
    """
    ...
    for i in range(len(chapters)):
        chapter = chapters[i]
        categories = categories_randomized[i]
        # Filter CSV for this chapter
        for cat in categories:
            chapter_segs = segments_df[(segments_df['pair_speaker'] == chapter) & (segments_df['category'] == cat)]
...
def main():
    ...
    with open(CHAPTERS_JSON_PATH, "r") as f:
        ch_dict = json.load(f)
    chapters = ch_dict['train'] + ch_dict['test']
    pose, audio = load_raw_files(chapters)

    print(f"Loaded {len(pose)} train sequences and {len(audio)} test sequences.")

    # --- Step 2: Parse CSV for Windows ---
    print("Parsing Speaker Segments CSV...")
    segments_df = pd.read_csv(SPEAKER_SEGMENTS_PATH)
    categories = segments_df['category'].unique()
    print(f"Found categories in CSV: {categories}")
    val_categories_randomized = np.random.choice(categories, size=(len(chapters),1), replace=True)
    train_categories_randomized = []
    for cat in val_categories_randomized:
        train_categories_randomized.append([c for c in categories if c != cat[0]])

    train_indices, train_labels = parse_csv_windows(train_categories_randomized, segments_df, chapters,  FPS)
    val_indices, val_labels = parse_csv_windows(val_categories_randomized, segments_df, chapters, FPS)
    ...
    train_ds_raw = RawExtractionDataset(pose, audio, train_indices, train_labels, WINDOW_SIZE)
    val_ds_raw = RawExtractionDataset(pose, audio, val_indices, val_labels, WINDOW_SIZE)
    ...
    for ep in range(1, FINETUNE_EPOCHS+1):
        ...
        with torch.no_grad():
            for vx, vy in val_loader:
                ...
                v_logits = clf(vx)
                v_loss = crit(v_logits, vy)
                val_losses.append(v_loss.item())
        avg_val_loss = np.mean(val_losses)
        print(f"Epoch {ep}/{FINETUNE_EPOCHS} - train loss: {np.mean(train_losses):.4f} - val loss: {avg_val_loss:.4f}", flush=True)
    ...
    best_model_name = max(results_dict.items(), key=lambda x: x[1]['val_accuracy'])[0]
    print("\nBest Model Detailed Report:", best_model_name.replace("_", " ").title())
    best_preds = {
        "neural_network": nn_preds,
        "svm": svc_preds,
